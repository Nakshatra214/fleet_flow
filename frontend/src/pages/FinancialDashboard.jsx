...
export default function FinancialDashboard({ user }) {
    ...
    const profitChart = {
        ...
        datasets: [
            { label: 'Revenue ₹', data: chartDays.map(d => d.revenue), backgroundColor: 'rgba(34,197,94,0.7)', borderRadius: 4 },
            { label: 'Expenses ₹', data: chartDays.map(d => d.expenses), backgroundColor: 'rgba(239,68,68,0.55)', borderRadius: 4 },
            { label: 'Profit ₹', data: chartDays.map(d => d.profit), backgroundColor: 'rgba(99,102,241,0.7)', borderRadius: 4 },
        ]
    };
    ...
    return (
        <div className="space-y-5">
            {/* Header */}
            <div className="flex items-center gap-4 bg-slate-800/80 border border-violet-500/20 rounded-2xl p-5">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-white">Financial Analyst Dashboard</h1>
                    <p className="text-slate-400 text-sm mt-0.5">Revenue, expenses & profitability analysis · {user?.name}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
                {[
                    { icon: DollarSign, label: 'Total Revenue (30d)', value: `₹${totalRevenue.toLocaleString()}`, sub: `${completedTrips.length} trips`, color: 'bg-green-600' },
                    { icon: Wrench, label: 'Total Expenses (30d)', value: `₹${totalExpenses.toLocaleString()}`, sub: `Fuel + Maint + Pay`, color: 'bg-red-600' },
                    { icon: TrendingUp, label: 'Net Profit (30d)', value: `₹${totalProfit.toLocaleString()}`, sub: profitMargin + '% margin', color: totalProfit >= 0 ? 'bg-indigo-600' : 'bg-rose-600' },
                    { icon: BarChart3, label: 'Fuel Costs', value: `₹${(stats?.finances?.totalFuelCost || 0).toLocaleString()}`, sub: 'All time total', color: 'bg-amber-600' },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="bg-slate-800/80 border border-slate-700/60 rounded-2xl p-5">
                    <p className="text-white font-semibold text-sm mb-3 flex items-center gap-2"><BarChart3 size={14} className="text-green-400" /> Revenue vs Expenses vs Profit</p>
                    <div className="h-52">
                        {chartDays.some(d => d.revenue > 0 || d.expenses > 0) ? (
                            <Bar data={profitChart} options={chartOpts} />
                        ) : (
                            <div className="h-full flex items-center justify-center text-slate-500 text-sm">No financial data yet</div>
                        )}
                    </div>
                </div>
                <div className="bg-slate-800/80 border border-slate-700/60 rounded-2xl p-5">
                    <p className="text-white font-semibold text-sm mb-3 flex items-center gap-2"><TrendingUp size={14} className="text-indigo-400" /> Profit Trend</p>
                    <div className="h-52">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                    { icon: Fuel, label: 'Total Fuel Cost', value: stats?.finances?.totalFuelCost || 0, color: 'text-amber-400 border-amber-400/20 bg-amber-400/5' },
                    { icon: Wrench, label: 'Total Maintenance Cost', value: stats?.finances?.totalMaintenanceCost || 0, color: 'text-orange-400 border-orange-400/20 bg-orange-400/5' },
                    { icon: DollarSign, label: 'Total Driver Pay', value: stats?.finances?.totalDriverPay || 0, color: 'text-indigo-400 border-indigo-400/20 bg-indigo-400/5' },
                ].map(({ icon: Icon, label, value, color }) => (
                    <div key={label} className={`border ${color} rounded-2xl p-5 flex items-center gap-4`}>
            ...
            <div className="bg-slate-800/80 border border-slate-700/60 rounded-2xl overflow-hidden">
                <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-700/60">
                    ...
                    <h3 className="text-white font-semibold text-sm">Recent Completed Trips (Revenue)</h3>
                ...
                <div className="divide-y divide-slate-700/40 max-h-64 overflow-y-auto">
                    {completedTrips.length === 0
                        ? <p className="text-slate-500 text-sm text-center py-8">No completed trips yet</p>
                        : completedTrips.map(t => (
                            <div key={t._id} className="flex items-center gap-3 px-5 py-3">
                                <div className="flex-1 min-w-0">
                                    <p className="text-white text-xs font-medium">{t.origin} <ChevronRight size={11} className="inline" /> {t.destination}</p>
                                    <p className="text-slate-500 text-xs">{t.driverId?.name} · {t.vehicleId?.name}</p>
                                </div>
                                <div className="text-right flex-shrink-0">
                                    <p className="text-green-400 text-xs font-bold">₹{(t.revenue || 0).toLocaleString()}</p>
